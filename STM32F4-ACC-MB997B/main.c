...
static THD_FUNCTION(AccThd, arg) {
  ...
  while (true) {
    ...
    chprintf(smShellGetHandler(SHELL_INTERFACE_UART), "LIS302DLD1 Raw: ");
    for(uint8_t i = 0; i < LIS302DL_ACC_NUMBER_OF_AXES; i++) {
      chprintf(smShellGetHandler(SHELL_INTERFACE_UART), "%c: %d ", axisID[i], accraw[i]);
    }

    lis302dlAccelerometerReadCooked(&LIS302DLD1, acccooked);
    chprintf(smShellGetHandler(SHELL_INTERFACE_UART), " Cooked: ");
    for(uint8_t i = 0; i < LIS302DL_ACC_NUMBER_OF_AXES; i++) {
      chprintf(smShellGetHandler(SHELL_INTERFACE_UART), "%c: %.3f ", axisID[i], acccooked[i]/1000);
    }
    chprintf(smShellGetHandler(SHELL_INTERFACE_UART),"\r\n");
...
int main(void) {
  ...
  smShellInitInterface(SHELL_INTERFACE_UART); // Change to SHELL_INTERFACE_CDC for CDC

  /* Creates the Acc thread.*/
  chThdCreateStatic(waAccThd, sizeof(waAccThd), NORMALPRIO + 1, AccThd, NULL);

  while (true) {
    smShellKeepAlive(SHELL_INTERFACE_UART); // Change to SHELL_INTERFACE_CDC for CDC
